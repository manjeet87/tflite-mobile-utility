  ...
  */
  ...
  fetchData = async () => {
    ...
    this.setState({loading:true})
     await fetch(global.mainserver+"/my_board_images", {
      method: 'GET',
      headers: {
  ...
  render() {
    ...
    return (
      ...
      <FlatList
          ...
          renderItem={({ item }) =>
          <TouchableOpacity onPress= {()=> this.props.navigation.navigate('detail_screen_board', {item})}>
                <ImageBackground style = {styles.image} source={{uri: item.imageurl}} resizeMode='cover' blurRadius={0.8}> 
                        <Text style= {styles.text}>{item.brand}</Text>
                        <Text style= {styles.text}>{item.status}</Text>
                        <Text style= {styles.text}>{item.outlet_name}</Text>
                </ImageBackground>
          </TouchableOpacity>        
